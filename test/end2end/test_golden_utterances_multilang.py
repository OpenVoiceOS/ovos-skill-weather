...
The skill is network-bound (Open-Meteo + geolocation); both boundaries are
monkeypatched module-wide for the duration of the run, same mechanism as
the existing en-US suite.
...
def _raise_no_network(*args, **kwargs):
    raise ConnectionError("network disabled for offline golden-utterance test")
